...
public abstract class VillagerBreedTaskMixin {
    ...
    @Shadow
    abstract boolean canReach(Villager body, BlockPos poiPos, Holder<PoiType> poiType);

    @Inject(method = "tryToGiveBirth(Lnet/minecraft/server/level/ServerLevel;Lnet/minecraft/world/entity/npc/villager/Villager;Lnet/minecraft/world/entity/npc/villager/Villager;)V",
    ...
    private void goHome(ServerLevel world, Villager first, Villager second, CallbackInfo ci) {
        if (CONFIG.villagersGeneralConfig.villagerBabiesRequireWorkstationAndBed) {
            Optional<BlockPos> optionalWorkstation = world.getPoiManager()
                    .take(VillagerProfession.ALL_ACQUIRABLE_JOBS,
    ...
    @ModifyConstant(
            method = "takeVacantBed(Lnet/minecraft/server/level/ServerLevel;Lnet/minecraft/world/entity/npc/villager/Villager;)Ljava/util/Optional;",
